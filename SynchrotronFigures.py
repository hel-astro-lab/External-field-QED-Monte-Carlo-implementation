...
if False:
    ...
    t_data = pd.read_csv('T_Synch_tab030226.csv')
    n_data = pd.read_csv('N_Synch_tab030226.csv')
   ...
   # for j in range(ncol_fig):
    ...
    #plt.savefig('AuxTSynch020426joinedxaxisjulkaisu.pdf')
...
#    plt.savefig('syncprobplot040126joinedxjulkaisu.pdf')
   ...
   # plt.savefig('syncSpectrumplots040226joinedxjulkaisu.pdf')
...
if False:
    ...
    fig.subplots_adjust(left=axleft, bottom=axbottom, right=axright, top=axtop)
    #plt.savefig('Synch2Dplots.5.2.26julkaisu.pdf')
    plt.show()
...
#    fig.savefig('SynchquarticPadePhotonChi.04.03.26julkaisu.pdf')
...
if False:
    for variable in ['Pn', 'Pa', 'Pb']:
        ...
        if variable == 'Pa':
            data = pd.read_csv('SynchInt1MPade(4;0,2,3)INFPFirstAandB030226.csv')
            plotcolor = 'C0'
            def model(x):
            ...
            data = pd.read_csv('SynchInt1MPade(4;0,2,3)INFPFirst030226.csv')
            plotcolor = 'C2'
            def model(x):
...
if False:
    ...
    fig.savefig('SynchABNparamplot100226julkaisu.pdf')
    plt.show()
...
if False:
    ...
    for j in range(ncol_fig):
        for i in range(nrow_fig):
            axs[i,j] = plt.subplot(gs[i,j])
            axs[i,j].minorticks_on()
            axs[i,j].set_xlabel('$\chi_\gamma/\chi_e$', fontsize=14)
            axs[i,j].set_yscale('log')
            #axs[i,j].set_xscale('log')
    ...
    for n, chi_in in enumerate([0.0005, 0.05, 0.5, 50]):
        ...
        for i in range(N):
            epsilon_r = 0.0005
            zeta = np.random.uniform(low=epsilon_r, high=(1 - epsilon_r))
    ...
    fig.subplots_adjust(left=axleft, bottom=axbottom, right=axright, top=axtop)
    #plt.savefig('SynchProducedSpectrajulkaisu.pdf')
    plt.show()
